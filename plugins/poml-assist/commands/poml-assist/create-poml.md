---
description: ユーザーへの質問を通じてPOMLプロンプトを対話的に設計・生成する
argument-hint: "[出力ファイルパス（省略可）]"
allowed-tools: Read, Write, Bash
---

# poml-assist: create-poml

ユーザーへの段階的な質問を通じて、高品質な POML プロンプトを対話的に設計・生成します。

## フェーズ 0: 引数確認

`$ARGUMENTS` を確認する:
- **引数あり**: 出力パスとして使用（例: `output.poml`）
- **引数なし**: フェーズ 1 の最後に出力先を確認する

## フェーズ 1: 基本情報の収集

以下の 3 つを一括で質問する（まとめて 1 回で聞く）:

```
POML プロンプトを作成します。以下の 3 点を教えてください:

1. **用途・目的**: このプロンプトで何をしたいですか？
   （例: テキスト要約、感情分類、コードレビュー、Q&A など）

2. **AIのペルソナ**: どのような役割・専門性の AI として振る舞うべきですか？
   （例: 「優秀なコードレビュアー」「カスタマーサポート担当者」など。不要なら「なし」）

3. **入力データ**: プロンプトに渡す入力データの種類は何ですか？
   （例: テキスト、コード、CSV ファイル、画像など）
```

引数なしの場合はここで出力先も確認:
```
4. **出力先ファイルパス**: 生成した POML を保存するファイルパスを教えてください。
   （例: `prompts/summarize.poml`、省略するとカレントディレクトリに保存）
```

## フェーズ 2: 出力形式と Few-shot の確認

```
ありがとうございます。続いて出力に関して確認させてください:

1. **出力形式**: AI にどのような形式で出力させたいですか？
   - テキスト（自由形式）
   - Markdown（見出しや箇条書き）
   - JSON（構造化データ）
   - その他（具体的に: ）

2. **Few-shot サンプル**: 入力→出力の具体例を提供しますか？
   （例示があると AI の精度が上がります）
   - はい → 例を教えてください（入力例と期待する出力例）
   - いいえ
```

## フェーズ 3: テンプレート変数の確認（オプション）

```
最後に、テンプレート変数について確認します:

このプロンプトを複数の用途で使い回すために、変数化したい部分はありますか？
（例: 対象言語、要約の長さ、トーンなどを変数にすると再利用しやすくなります）

- はい → 変数名とデフォルト値を教えてください
- いいえ → このまま進みます
```

## フェーズ 4: POML 生成

収集した情報を `poml-architect` エージェントに渡して POML を生成する。

エージェントに渡す情報を整理する:
- 用途・目的
- ペルソナ（あれば）
- 入力データの種類
- 出力形式
- Few-shot サンプル（あれば）
- テンプレート変数（あれば）

生成した POML をプレビュー表示する:

```
以下の POML を生成しました:

---
[生成された POML の内容]
---

この内容でよろしいですか？
- はい → ファイルに保存します
- いいえ → 修正したい点を教えてください
```

修正が必要な場合はフェーズ 4 を繰り返す。

## フェーズ 5: ファイル保存

ユーザーが承認したら、決定した出力パスに POML を書き出す。

```
✓ POML を保存しました: [ファイルパス]

ファイルが保存されると、自動的にバリデーションとレンダリングプレビューが実行されます。
```

## フェーズ 6: レビュー案内

```
保存が完了しました。次のステップ:

1. **レビュー**: `poml-reviewer` エージェントにレビューを依頼できます
   → 「POML ファイルをレビューして」と話しかけてください

2. **バリデーション**: CLI でも確認できます
   → `poml check [ファイルパス]`

3. **レンダリング確認**: プロンプトのテキスト変換を確認できます
   → `poml render [ファイルパス]`

別の POML を作成しますか？
```

## 重要な原則

- **各フェーズで待機する**: ユーザーの回答を受け取ってから次のフェーズに進む
- **修正に柔軟に対応**: フェーズ 4 のプレビューで修正依頼があった場合は、具体的にどこを修正するかを確認してから再生成する
- **引数が指定された場合**: フェーズ 0 で出力パスを確定し、フェーズ 1 の質問 4 はスキップする
